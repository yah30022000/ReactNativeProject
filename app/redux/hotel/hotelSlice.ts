...
import {
  HotelListRequest,
  HotelListResponse,
} from "../../helper/amadeus/hotel-list-request-response";
...
export const getAmadeusAccessToken = createAsyncThunk<
  GetAccessTokenResponse,
  GetAccessTokenRequest
>(
  ...
  async (requestBody: GetAccessTokenRequest, thunkAPI) => {
    ...
    const response = await fetch(
      `${amadeusTestApiUrl}/v1/security/oauth2/token`,
      {
        method: "POST",
        headers: {
...
export const {chooseCityCode, selectRating} = hotelSlice.actions;
